...
const ModifyEquipment = () => {
  ...
  useEffect(() => {
    const fetchEquipmentData = async () => {
      try {
        const response = await axios.get(`https://noderole-1.onrender.com/equip/equip/${id}`);
  ...
  const handleModifyEquipment = async (values) => {
    try {
      const response = await axios.put(`https://noderole-1.onrender.com/equip/equip/${id}`, values);
