...
const Intervention = () => {
  ...
  useEffect(() => {
    const fetchEquipments = async () => {
      try {
        const { data } = await axios.get('https://noderole-1.onrender.com/equip');
        setEquipments(data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchInterventions = async () => {
      try {
        const { data } = await axios.get(`https://noderole-1.onrender.com/api/interventions/search?search=${search}`);
  ...
  const handleAddIntervention = async (values) => {
    try {
      const response = await axios.post('https://noderole-1.onrender.com/api/interventions', values);
