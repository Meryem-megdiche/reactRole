...
const Team = () => {
  ...
  const handleButtonClick = async (row) => {
    ...
    try {
      const response = await axios.get(`https://noderole-1.onrender.com/api/config/isConfigured/${row.id}`);
  ...
  const handleButton2Click = async (row) => {
    try {
      ...
      await axios.delete(`https://noderole-1.onrender.com/equip/${row.id}`);
      ...
      try {
        const updatedData = await axios.get("https://noderole-1.onrender.com/equip");
  ...
  const handlePingButtonClick = async (row) => {
    ...
    try {
      const response = await axios.post(`https://noderole-1.onrender.com/pingtest/manual`, { ip: equipIp, equipId });
